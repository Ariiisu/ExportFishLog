...
void xivres::texture::preview(const stream& texStream, std::wstring title) {
	...
	struct State {
		...
		void LoadMipmap(int repeatIndex, int mipmapIndex, int depthIndex) {
			...
			bmih.biWidth = strm->Width;
			bmih.biHeight = -strm->Height;
			bmih.biPlanes = 1;
			bmih.biBitCount = 32;
		...
		void Draw(HDC hdc, const RECT& clip) {
			...
			const auto dw = static_cast<int>(strm->Width * zoom);
			const auto dh = static_cast<int>(strm->Height * zoom);
			...
			switch (msg) {
				...
				case WM_MOUSEMOVE:
				{
					if (state.dragging) {
						...
						const auto zwidth = static_cast<int>(state.strm->Width * state.GetZoom());
						const auto zheight = static_cast<int>(state.strm->Height * state.GetZoom());
